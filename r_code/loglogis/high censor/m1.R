...
dyn.load("~/Documents/spatial_temporal/c_code/loglogis_m1.so")
require(MASS)
require(doMC)
registerDoMC(cores=6)
idx<-as.numeric(Sys.getenv("LSB_JOBINDEX"))
filname<-paste("loglogis_m1_s",idx,".RData", sep="")


##likelihood###
LL=function(b0,b1,b2,sig,W,y,s,x,x2,year,region_code){
    m=b0+b1*x+W[region_code,1]+b2*x2
    lam=exp(-m/sig)
    a=1/sig
...
estimate=foreach(n.sim=1:nsim,.errorhandling=c('remove')) %dopar% {
 ...
 # b_t=(1/lambda_t)^(1/a_t)
  ...
  result=.C("weibul",b0=as.double(b0_sample), b1=as.double(b1_sample), 
            ...
            x2=as.double(x2), year=as.double(year+1), Cores=as.integer(region_code-1),
            nend=as.integer(mc_sample), no_regions=as.integer(region),dims=as.integer(c(p,p)),
            alpha_gamma=as.double(0.001),beta_gamma=as.double(1000),deviance=as.double(0),
  ...
  D_theta_bar=-2*LL(param_mean$b0,param_mean$b1,param_mean$b2, param_mean$sig,W_means,y,status,x,x2,year+1,region_code)
