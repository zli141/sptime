...
double target_b0 (double x, double b1, double b2,  double *phi1, double sig, int *Cores, double *year, double *x1, double *x2, double *y, int *s, int N)
{
    double sum=0.0,mu=0.0,lam=0.0,a= 0.0;
    for (int i = 0; i <N; i++)
    {
        mu=x + b1*x1[i]+b2*x2[i]+phi1[Cores[i]];
        lam=exp(-mu/sig);
        a=1/(sig);
...
double target_b1 (double b0, double x, double b2, double *phi1, double sig, int *Cores, double *year, double *x1, double *x2, double *y, int *s, int N)
{
    double sum=0.0,mu,lam,a= 0.0;
    for (int i = 0; i < N; i++)
    {
        mu=b0 + x*x1[i]+b2*x2[i]+phi1[Cores[i]];
        lam=exp(-mu/sig);
        a=1/(sig);
        sum=sum+s[i]*log(lam*a*pow(y[i],(a-1)))-lam*pow(y[i],a);
...
double target_b2 (double b0, double b1, double x, double *phi1, double sig, int *Cores, double *year, double *x1, double *x2, double *y, int *s, int N)
{
    double sum=0.0,mu,lam,a= 0.0;
    for (int i = 0; i < N; i++){

        mu=b0 + b1*x1[i]+x*x2[i]+phi1[Cores[i]];
        lam=exp(-mu/sig);
        a=1/(sig);
        sum=sum+s[i]*log(lam*a*pow(y[i],(a-1)))-lam*pow(y[i],a);
...
double target_phi(double b0, double b1, double b2, double *phi1, double sig,int *Cores, double *year, double *x1, double *x2, double *y, int *s, int N, int no_regions, double **RTR11P1, double *lamda, double x1_new, int k)
{
    ...
    for (i = 0; i < N; i++)
    {
        mu=b0+b1*x1[i]+b2*x2[i]+temp1[Cores[i]];
        lam=exp(-mu/sig);
        a=1/(sig);
        sum=sum+s[i]*log(lam*a*pow(y[i],(a-1)))-lam*pow(y[i],a);
...
double target_sig (double b0, double b1, double b2, double *phi1, double x, int *Cores, double *year,double *x1, double *x2,double *y, int *s, int N)
{
    double sum=0.0,mu,lam,a= 0.0;
     a=1/(x);
    ...
    for (int i = 0; i < N; i++)
    {
        mu=b0 + b1*x1[i]+b2*x2[i]+phi1[Cores[i]];
        lam=exp(-mu/x);
        sum=sum+s[i]*log(lam*a*pow(y[i],(a-1)))-lam*pow(y[i],a);
    }
...
double metroplis_b0(double x1_prev, double b1,double b2,double *phi1, double sig,  int *Cores, double *year,double *x1, double *x2,double *y, int *s, int N )
{
    ...
    temp1=target_b0(y1,b1,b2,phi1, sig,Cores,year,x1,x2,y,s,N);
    temp2=target_b0(x1_prev,b1,b2,phi1,sig,Cores,year,x1,x2,y,s,N);
    lgratio=temp1-temp2;
    if (u>0)
    {
...
double metroplis_b1(double b0, double x_prev,double b2,double *phi1, double sig, int *Cores, double *year,double *x1, double *x2,double *y, int *s, int N)
{
    double lgratio=0, u ,y_new, x=0, temp1=0, temp2=0;
    u=runif(0,1);
    y_new=x_prev+0.05*rnorm(0,1);
  
    temp1=target_b1(b0,y_new,b2,phi1,sig,Cores,year,x1,x2,y,s,N);
    temp2=target_b1(b0,x_prev,b2,phi1,sig,Cores,year,x1,x2,y,s,N);
    lgratio=temp1-temp2;
    if (u>0)
    {
...
double metroplis_b2( double b0, double b1, double x_prev,double *phi1,double sig, int *Cores, double *year,double *x1, double *x2,double *y, int *s, int N)
{
    double lgratio=0, u ,y_new, x=0, temp1=0, temp2=0;
    u=runif(0,1);
    y_new=x_prev+0.05*rnorm(0,1);
    temp1=target_b2(b0,b1,y_new,phi1,sig,Cores, year,x1,x2,y,s,N);
    temp2=target_b2(b0,b1,x_prev,phi1,sig,Cores, year,x1,x2,y,s,N);
    lgratio=temp1-temp2;
    if (u>0)
    {
        if (lgratio>=log(u))  x=y_new ;
...
void metroplis_phi(double b0, double b1,double b2,double *phi_curr1, double sig, double *lamda, int *Cores, double *year, double *x1, double *x2, double *y,int *s, int N, int no_regions, double **RTR11P1,double x1_prev,  int j)
{
   ...
   // if(j==0) printf("x2_curr is %f \n", x2_curr);
    ...
    temp1=target_phi(b0,b1,b2,phi_curr1,sig, Cores, year, x1,x2,y,s,N, no_regions,RTR11P1,lamda, x1_curr,j);
    temp2=target_phi(b0,b1,b2,phi_curr1,sig, Cores, year, x1,x2,y,s,N, no_regions,RTR11P1,lamda, x1_prev,j);
   lgratio=temp1-temp2;
...
double metroplis_sig( double b0, double b1, double b2, double *phi_curr1, double x_prev, int *Cores, double *year,double *x1, double *x2, double *y,int *s, int N)
{
    double lgratio=0, u ,y_new, x=0, temp1=0, temp2=0;
    u=runif(0,1);
    y_new=x_prev+rnorm(0,0.1);
    if (y_new>0){
    temp1=target_sig(b0,b1,b2,phi_curr1,y_new,Cores,year,x1,x2,y,s,N);
   
    temp2=target_sig(b0,b1,b2,phi_curr1,x_prev,Cores,year,x1,x2,y,s,N);
...
double LL (double b0, double b1, double b2, double *phi1,  double sig, int *Cores, double *year, double *x1, double *x2, double *y, int *s, int N)
{
    double sum=0.0,mu,lam,a= 0.0;
    for (int i = 0; i < N; i++){
        mu=b0 + b1*x1[i]+b2*x2[i]+phi1[Cores[i]];
...
void weibul(double *b0, double *b1, double *b2, double *sig, double *lamda, double *phi1,  double *vecD, double *vecC,double *y, int *s, int *N_p, double *x1, double *x2, double *year, int *Cores, int *nend, int *no_regions_p,int *dims,double *alpha_gamma,double *beta_gamma,double *sumLnF,double *phi_sample,int *n_burn){
	int i, j;
    int p=dims[0];
    int N=N_p[0];
   ...
   for (i = 1; i < nend[0]; i++)
     //for (i = 1; i < 2; i++)
	{
	 ...
	 b0[i]=metroplis_b0(b0[i-1],b1[i-1],b2[i-1],phi1,sig[i-1],Cores,year,x1,x2,y,s,N);
     b1[i]=metroplis_b1(b0[i],b1[i-1],b2[i-1],phi1,sig[i-1],Cores,year,x1,x2,y,s,N);
     b2[i]=metroplis_b2(b0[i],b1[i],b2[i-1],phi1,sig[i-1],Cores,year,x1,x2,y,s,N);
        for (j = 0; j < no_regions; j++)
	    {
            x1_prev=phi1[j];
           
            metroplis_phi(b0[i],b1[i],b2[i],phi1,sig[i-1],lamda, Cores,year,x1,x2,y,s,N,no_regions,RTR11P1,x1_prev,j);
    ...
    //printf("Before center %d is %f %f %f \n",i+1,phi2[0],phi2[1],phi2[2]);
      ...
      sig[i]=metroplis_sig(b0[i],b1[i],b2[i],phi1,sig[i-1],Cores,year,x1,x2,y,s,N);

      lamda_region=target_lamda(phi1,no_regions,RTR11P1);
      lamda[0]=rgamma(0.001+no_regions/2,1/(0.001+1/2*lamda_region));
      
      if(i>=n_burn[0]) sumLnF[0]=sumLnF[0]+LL(b0[i],b1[i],b2[i],phi1,sig[i],Cores,year,x1,x2,y,s,N);
